        ...
        <div class="x_content">
            ...
            if (isset($_POST["addNew"])) {
                ...
                if (isset($_GET["id"]) && isset($_GET["edit"])) {
                    //Sửa theo id
                    $sql = "UPDATE size SET size_name='$sizeName', `status`='$status',date_create='$date_create' WHERE size_id =" . $_GET["id"];
                    mysqli_query($conn, $sql) or ("Lỗi câu lệnh cập nhật ");
                    header("localhost:index.php?page=size");
                } else {
            ...
            if (isset($_GET["id"]) && isset($_GET["edit"])) { //Lấy id
                $sql = "SELECT * FROM size WHERE size_id=" . $_GET["id"]; //Lấy dữ liệu qua id
                $result = mysqli_query($conn, $sql); //Hàm trả về giá trị
                $row = mysqli_fetch_row($result);
                // echo "<pre>";
                // print_r($row);
                $size_name = $row[1]; //Gán dữ liệu
            ...
            if (isset($_GET["id"]) && isset($_GET["del"])) {
                $sql = "DELETE FROM size WHERE size_id=" . $_GET["id"];
            ...
            <form class="form-label-left input_mask" method="post">
                ...
                <div class="form-group row">
                    <label class="col-form-label col-md-3 col-sm-3 ">Tên kích cỡ</label>
                    <div class="col-md-9 col-sm-9 ">
                        <input type="text" class="form-control" value="<?php echo $size_name; ?>" name="size_name" id="size_name" placeholder="Tên kích cỡ">
                    </div>
                </div>
                <div class="form-group row">
